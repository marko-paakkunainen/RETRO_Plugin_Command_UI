Public Module modMain
    ...
    Public gstrProjectDir As String = ""
    ...
    Public Function DqtEsc(pstrInput As String, Optional pblnVB_JSON As Boolean = True) As String
        If pblnVB_JSON Then
            Return Chr(34) & Replace(Replace(pstrInput, "\", "\\"), """", "\""") & Chr(34)
        Else
            Return Chr(34) & Replace(pstrInput, """", "\""") & Chr(34)
        End If
    End Function

End Module

Public Module FileSys

    Public Function GetFiles(pstrPath As String, Optional pblnReturnFullPath As Boolean = False) As List(Of String)
        Return GetFiles(pstrPath, "", pblnReturnFullPath)
    End Function

    Public Function GetFiles(pstrPath As String, pstrExtension As String, Optional pblnReturnFullPath As Boolean = False) As List(Of String)
        ...
        If Not pblnReturnFullPath Then
            ...
            If pstrExtension <> "" Then
                For Each strFile In objFiles
                    If LCase(GetFileExt(strFile)) = LCase(pstrExtension) Then
        ...
        Else
            If pstrExtension <> "" Then
                For Each strFile In objFiles
                    If LCase(GetFileExt(strFile)) = LCase(pstrExtension) Then lstBuild.Add(strFile)
                Next
            Else
                For Each strFile In objFiles
                    lstBuild.Add(strFile)
    ...
    Public Function GetFileName(pstrFullName As String, Optional pblnExtension As Boolean = True) As String
        Dim arrTmp As Array = Split(pstrFullName & "\", "\")
        Dim sb As System.Text.StringBuilder = New System.Text.StringBuilder
        If arrTmp.Length > 1 Then
            sb.Append(arrTmp(arrTmp.Length - 2))
            If pblnExtension = False Then
                If InStr(sb.ToString, ".") Then
                    arrTmp = Split(sb.ToString, ".") : sb.Clear()
                    For i = 0 To arrTmp.Length - 2
                        sb.Append(arrTmp(i))
                    Next
                End If
            End If : GetFileName = sb.ToString
        Else : GetFileName = pstrFullName
        End If
    End Function

    Public Function ReadFile(pstrFile As String, ByVal Optional utf8 As Boolean = False) As String
    ...
    Public Function GetFileExt(pstrFileName As String) As String
        If InStr(pstrFileName, ".") > 0 Then
            Dim arrTmp As Array = Split(pstrFileName, ".")
            GetFileExt = arrTmp(arrTmp.Length - 1)
        Else : GetFileExt = "" : End If
    End Function

    Public Function GetPathFromFile(pstrFileName As String) As String
        Dim strBuild As String = Replace(pstrFileName, GetFileName(pstrFileName), "")
